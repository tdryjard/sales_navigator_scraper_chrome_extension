(function () {
  ...
  XMLHttpRequest.prototype.send = function (...args) {
    ...
    const isListProfils =
      this._url.includes("sales-api/salesApiPeopleSearch") ||
      this._url.includes("sales-api/salesApiLeadSearch");
    const isProfilpage = this._url.includes("/sales-api/salesApiProfiles/(profileId");
    if (isListProfils || isProfilpage) {
      ...
      this.addEventListener("load", function () {
        ...
        if (this.responseType === "blob" || this.response instanceof Blob) {
          ...
          reader.onload = () => {
            ...
            window.postMessage(
              {
                type: isListProfils ? "FROM_PAGE" : "FROM_PROFIL",
